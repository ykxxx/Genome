...
const string PROVIDED_DIR = "/Users/ykx/Desktop/CS32/Project/Project4/Gee-nomics/data";
...
//const string providedFiles[] = {"test.txt"};
...
{
    if (exactMatch)
        cout << "Enter DNA sequence for which to find exact temp: ";
    else
        cout << "Enter DNA sequence for which to find exact temp and SNiPs: ";
    string sequence;
    getline(cin, sequence);
    ...
    if (!library->findGenomesWithThisDNA(sequence, minMatchLength, exactMatch, temp))
    {
        cout << "No ";
        if (exactMatch)
            cout << " temp";
...
void findRelatedGenomesManual(GenomeMatcher* library)
{
    ...
    vector<GenomeMatch> temp;
    library->findRelatedGenomes(Genome("x", sequence), 2 * minLength, exactMatchOnly, pctThreshold, temp);
    if (temp.empty())
    {
        cout << "    No related genomes were found" << endl;
        return;
    }
    cout << "    " << temp.size() << " related genomes were found:" << endl;
    cout.setf(ios::fixed);
    cout.precision(2);
    for (const auto& m : temp)
...
{
    string filename;
    cout << "Enter name of file containing one or more genomes to find temp for: ";
    getline(cin, filename);
    if (filename.empty())
    {
        cout << "No file name entered." << endl;
    ...
    {
        vector<GenomeMatch> temp;
        library->findRelatedGenomes(g, 2 * minLength, exactMatchOnly, pctThreshold, temp);
...
void showMenu()
{
    ...
    cout << "         e - find temp exactly           q - quit" << endl;
}

int main()
{
    const int defaultMinSearchLength = 5;
